...
**Rejected 库：** `~/.agent-reach/daily_run/rejected_strategies.jsonl` — 已证伪策略禁止写回 playbook / settings（周六审视自动过滤）；周六 weekly 刷新归档至 `rejected_strategies_archive.jsonl`，仅**当周**条目阻断买入（`rejected_strategies.active_week_only`）。
