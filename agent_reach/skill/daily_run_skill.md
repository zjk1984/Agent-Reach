...
本技能定义了如何作为**股票大师**，结合 **agent-reach** 的网页抓取能力定位全球宏观与微观因子，并调用 **daily_stock_analysis (DSA)** 执行自动化 K 线拉取、多市场共振与技术面融合的 AI 决策分析。
...
|  [量化调仓]    T1 ──────> T2 ──────> T3 ──────> T4 ──────> T5                      |
|                ^          ^          ^          ^          ^                      |
|  [时间决定]  (随机数 + 评估分值共同决定下一次 T_n 的精确触发秒数)                   |
+-----------------------------------------------------------------------------------+
```
...
### 3. 调仓时间由随机数与上次评估综合决定
为了防止被主力资金算法“反向猎杀”（Anti-Frontrunning），每次调仓的精确时间（分/秒）不固定，由以下公式决定：
$$\text{Next\_Trade\_Time} = \text{Base\_Interval} + f(\text{Last\_MSS}) + \text{Random\_Delay}(\text{Seed})$$
*   **Last_MSS (上次评估分值)：** 若市场波动加剧（分值极高或极低），缩短等待时间（Base_Interval 变小），加快调仓频率；若市场处于横盘震荡，拉长等待时间。
*   **Random_Delay：** 引入高强度随机数扰动（如 1-15 分钟的随机延迟），使交易时间分布呈现不规则性，达到完美潜伏的效果。
