...
本技能定义了如何作为**股票大师**，结合 **agent-reach** 的网页抓取能力定位全球宏观与微观因子，并调用 **daily_stock_analysis (DSA)** 执行自动化 K线拉取、多市场共振与技术面融合 of AI 决策分析。
...
*   **第一步：Agent-Reach 权限自检 (Auth Check)：**
    ...
    *   **白名单过滤：** 自动**排除小红书**（避免无意义的扫码或由于服务器环境导致的报错）。
